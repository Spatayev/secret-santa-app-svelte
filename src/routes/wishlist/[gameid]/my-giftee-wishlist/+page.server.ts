...
export const actions = {
	get_my_wishlist: async ({ request, params, cookies  }) => {
		const form = await superValidate(request, zod(schema));

		console.log('get_my_wishlist');
		...
		try {
		const response = await fetch(`http://158.160.21.73:8080/wishlist/${params.gameid}/my-giftee-wishlist`, {
			method: 'GET',
			headers: {
				'accept': '*/*',
